...
def generate_ass_subtitles(
    ...
    countdown_style: str = "hearts",       # Countdown style: "hearts", "smileys", "dots", "numbers"
    countdown_count: int = 4,              # Countdown count: 4 or 3
    show_countdown: bool = True            # Enable lead-in countdown cues
) -> str:
    r"""
    ...
    # Countdown Cues configuration
    if countdown_style == "hearts":
        cd_primary = "&H002020FF"   # Red (BGR)
        cd_secondary = "&H665050FF" # Soft dim red
        cd_symbol = "♥"
    elif countdown_style == "smileys":
        cd_primary = "&H0033D2FF"   # Golden Yellow (BGR)
        cd_secondary = "&H6620A0FF"
        cd_symbol = "☻"
    elif countdown_style == "dots":
        cd_primary = "&H0059E2FF"   # Gold (BGR)
        cd_secondary = "&H66FFFFFF"
        cd_symbol = "●"
    else: # numbers
        cd_primary = "&H00F8BD38"   # Cyan (BGR)
        cd_secondary = "&H66FFFFFF"
        cd_symbol = None

    cd_fs = max(24, int(fs1 * 0.62))
    margin_cd = margin_v1 + int(fs1 * 0.95)
    align_cd = align1_val
    margin_l_cd = margin_l1 if align_cd == 1 else margin_lr

    cd_count = max(3, min(4, int(countdown_count or 4)))
    k_duration = int(200 / cd_count)
    if cd_symbol:
        cd_text = " ".join([f"{{\\k{k_duration}}}{cd_symbol}" for _ in range(cd_count)])
    else:
        nums = ["4", "3", "2", "1"] if cd_count == 4 else ["3", "2", "1"]
        cd_text = " ".join([f"{{\\k{k_duration}}}{n}" for n in nums])
...
Style: CountdownDotStyle,{font_name},{cd_fs},{cd_primary},{cd_secondary},{outline_color},{shadow_color},-1,0,0,0,100,100,1,0,1,{outline_val},{shadow_val},{align_cd},{margin_l_cd},{margin_lr},{margin_cd},1
...
"""
    ...
    if display_mode == "couplet":
        ...
        for p_idx, pair in enumerate(couplet_pairs):
            ...
            cd_lead_min = 1.6 if cd_count == 3 else 2.0
            if show_countdown and lead_in_sec_a >= cd_lead_min:
                cd_start = max(pair_lead_in, segA["start"] - (1.8 if cd_count == 3 else 2.0))
                cd_end = segA["start"]
                events.append(f"Dialogue: 1,{format_ass_time(cd_start)},{format_ass_time(cd_end)},CountdownDotStyle,,0,0,0,,{cd_text}")
        ...
        for st_idx, stanza in enumerate(stanzas):
            ...
            # Lead-in Countdown Dots Event (Hearts / Smileys / Dots / Numbers) for long intros / interludes
            cd_lead_min = 1.6 if cd_count == 3 else 1.8
            if show_countdown and (s0["start"] - stanza_entry) >= cd_lead_min:
                cd_start = max(stanza_entry, s0["start"] - (1.8 if cd_count == 3 else 2.0))
                cd_end = s0["start"]
                events.append(f"Dialogue: 1,{format_ass_time(cd_start)},{format_ass_time(cd_end)},CountdownDotStyle,,0,0,0,,{cd_text}")
